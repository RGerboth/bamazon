	...
	inquirer.prompt([
		...
		name: "quantityOrdered"
			...
			if (response.productOrdered && response.quantityOrdered) {
				...
				console.log(" ")
				console.log("Sorry, " + response.productOrdered + " " + response.quantityOrdered + " is not a valid order. You must specify a valid product and quantity.")
				placeOrder() 
			}
		})
	}
...
inquirer.prompt([ //place this whole thing inside placeOrder
	...
	if (response.confirm) {
		...
		placeOrder() //insert current placeOrder functions here
